...
**The filesystem is shared memory across iterations.** Specs live in the target project's `<project_dir>/openspec/changes/<change_name>/` (`proposal.md` / `tasks.md` / `specs/**/*.md`, plus `design.md` when needed), written by `analyze_plan` and read by `human_confirm` (via `_read_change_artifacts`), `execute`, `review`, and `archive`. The change name is asked once at the terminal (`_ask_change_name()`, not through Claude), only on the very first initial-planning call, and persisted in `AgentState["change_name"]` — if left blank, Claude falls back to the current git branch name. Review reports land in `docs/nodes/review/YYYY-MM-DD-iterN.md`. When debugging why a node picked up (or missed) prior context, check the OpenSpec change directory before assuming it's a prompt bug.
