...
**The filesystem is shared memory across iterations.** Plans land in `docs/superpowers/plans/YYYY-MM-DD-*.md` (written by `analyze_plan`, read by `human_confirm`/`review` via `project_context.latest_plan_file()` — glob-by-mtime, not by name); review reports land in `docs/nodes/review/YYYY-MM-DD-iterN.md`. When debugging why a node picked up (or missed) prior context, check these directories before assuming it's a prompt bug.
